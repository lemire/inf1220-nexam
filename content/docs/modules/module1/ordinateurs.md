...
Les années 1960 sont aussi celles d’une prise de conscience douloureuse : la «&nbsp;crise du logiciel&nbsp;». Alors que la puissance des ordinateurs progressait rapidement, les projets logiciels, eux, dépassaient systématiquement les budgets et les échéanciers, livraient des programmes truffés d’erreurs, ou n’étaient tout simplement jamais terminés. L’expression est consacrée lors de la conférence de l’OTAN sur le génie logiciel tenue à Garmisch, en Allemagne, en 1968&nbsp;: on y constate que l’on sait construire des machines beaucoup plus vite que l’on sait écrire des programmes fiables. C’est de ce constat que naît la discipline du génie logiciel, avec l’idée que l’écriture de programmes doit obéir à des méthodes rigoureuses plutôt qu’à l’improvisation.

Le mathématicien néerlandais Edsger Dijkstra (1930-2002) est l’une des figures marquantes de cette période. En 1968, il publie une courte lettre devenue célèbre, «&nbsp;Go To Statement Considered Harmful&nbsp;», dans laquelle il soutient que l’instruction de saut `goto` rend les programmes pratiquement impossibles à comprendre&nbsp;: en sautant librement d’un point à un autre du code, le programmeur perd la correspondance entre le texte du programme et son déroulement dans le temps. Dijkstra propose plutôt de n’utiliser qu’un petit nombre de structures de contrôle bien délimitées&nbsp;— la séquence, la sélection (`if`) et la répétition (`while`)&nbsp;— dont on peut raisonner le comportement. C’est le principe de la *programmation structurée*, popularisé par l’ouvrage *Structured Programming* (1972) qu’il cosigne avec Ole-Johan Dahl et C. A. R. Hoare.

{{< figure src="/images/dijkstra.jpg" alt="Edsger Dijkstra devant du code truffé de goto" >}}

L’argument de Dijkstra dépasse la simple question du `goto`. Pour lui, la programmation est avant tout une activité intellectuelle qui doit composer avec les limites de l’esprit humain&nbsp;: puisque nous ne pouvons pas garder en tête un nombre illimité de détails, il faut structurer les programmes de manière à pouvoir les comprendre par morceaux, indépendamment les uns des autres. Il défendait aussi l’idée que l’on devrait pouvoir démontrer la correction d’un programme, plutôt que de se contenter de le tester&nbsp;: «&nbsp;les tests peuvent démontrer la présence de bogues, jamais leur absence&nbsp;». Ces idées ont profondément marqué la conception des langages qui ont suivi. Le langage Pascal (1970), créé par Niklaus Wirth, a été explicitement conçu pour enseigner la programmation structurée, et les langages modernes comme Java, que nous étudierons dans ce cours, en sont les héritiers directs&nbsp;: ils offrent des blocs, des boucles, des méthodes et des classes, mais plus d’instruction `goto`.
